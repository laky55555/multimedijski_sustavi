...
class Window{
  ...
  void drawCurrentStage(){
    ...
    if(wellcome){
      ...
      drawer.makeText("Select number of players...", 20, 255, width/2, height/4 + 40);
    ...
    if(theEvent.getController().getName() == "Next"){
      ...
      if(playerNum == 0){
        ...
        for(int i = 0; i < playerNum; ++i){
          controlP5.addTextfield("Player"+i)
          ...
          commands[i+playerNum] = controlP5.addTextfield(i+"R")
                   .setPosition(width/2+180, height/4+80+i*50)
                   .setSize(30, 30)
                   .setFont(drawer.getControlFont(20));
